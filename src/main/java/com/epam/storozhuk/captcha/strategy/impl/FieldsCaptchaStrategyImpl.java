...
public class FieldsCaptchaStrategyImpl extends CaptchaStrategy {
    private static final Logger LOGGER = Logger.getLogger(FieldsCaptchaStrategyImpl.class);

    public FieldsCaptchaStrategyImpl(Map<String, CaptchaDescriptor> mapOfCaptchas) {
        super(mapOfCaptchas);
    }
    ...
    public void initCaptchaValue(HttpServletRequest request, HttpServletResponse response) {
        ...
        request.setAttribute(Const.HASHED_NUMBER_ATTRIBUTE, writeInitializedValueToMap());
    }

    @Override
    public Captcha writeValueToCaptcha(HttpServletRequest request, HttpServletResponse response) {
        String number = request.getParameter(Const.CAPTCHA_RIGHT_PARAMETER);
